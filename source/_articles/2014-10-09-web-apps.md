...
PHP is a language built specifically for the web. Most languages are general purpose.
...
Conversely, other languages have a process of translating a request into code. This usually means parsing the bytes constituting HTTP request data. Many libraries have been created as an add-on to most languages to handle requests. Newer languages can handle HTTP requests directly, but don't assume they are run in the HTTP context.

This is more like using cURL within PHP to accept web requests, rather than starting off with all the HTTP data available.

On a practical level, this means that when PHP is run, its super globals (`$_SERVER`, `$_GET`, `$_POST`, `$_SESSION`, `$_COOKIE` and so on) are already populated. PHP doesn't need to do work to get this data.
...
Modern applications, however, are not often simple. For more structured applications ("Enterprise"), we often need to deal with HTTP responses in great detail. Perhaps we want to encrypt cookies, or set cache headers. This often requires us to write objects describing HTTP. Then we can use those objects to adjust state and use PHP's HTTP-related functions to do things like set response headers and data.

Examples of this includes Symfony's HTTP libraries, and the possible PSR-7 standard.

For the end-user (developer), that's a subtle (hidden!) difference between PHP and other languages. Languages like Python and Ruby have a specification for how HTTP information should be sent. It's up to frameworks & libraries to handle getting/setting Cookies, Sessions and so on. Code manipulating response data must be written to send responses in the proper HTTP format.

On other hand, PHP frameworks simply manipulate HTTP request state given. PHP even has built-in methods to let you adjust HTTP response headers and data. This means PHP developers don't need to directly concern themselves with how data gets translated into a HTTP response. Instead, that's a concern for the internals members.
