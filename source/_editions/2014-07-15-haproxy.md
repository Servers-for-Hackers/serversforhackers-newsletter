...
* `http-request add-header X-Forwarded-Proto https if { ssl_fc }` - We add the `X-Forwarded-Proto` header and set it to "https" if the "https" scheme is used over "http". Similar to the forwarded-port header, this can help our web applications determine which scheme to use when building URL's and sending redirects (Location headers).
