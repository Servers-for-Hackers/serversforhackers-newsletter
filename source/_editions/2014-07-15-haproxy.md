...
We can accomplish sticky sessions with this algorithms. Sticky sessions means that user sessions (identified by a cookie) will inform HAProxy to always send requests from a client to a same server.

This is useful for web applications that use default session handling, which likely saves session data on the server, rather than within a cookie on the clients browser or in a centralized session store, such as redis or memcached. The implication being that users must be returned to the same server on which they created their session ("logged in") in order to remain in that session.

To do so, you can add a `cookie SOME-COOKIE-NAME prefix` directive into the backend. Then simply add the `cookie` directive within each server. Then HAProxy will append a cookie identifier (or add an identifier onto an existing cookie) for each server.
