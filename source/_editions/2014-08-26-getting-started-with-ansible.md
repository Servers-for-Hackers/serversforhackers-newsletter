...
I've found it to be one of the simplest and the easiest to get started with. A lot of this is because it's "just SSH". It uses SSH to connect to machine and run its tasks.

We often use SSH or other tools to run commands or bash scripts in order to provision and configure our servers. One nice thing about Ansible is that it's very easy to convert bash scripts (or a series of commands) into Ansible tasks. Since it's primarily SSH based, it's not hard to see why this might be the case - Ansible ends up running the same commands.
...
Of course we need to start by installing Ansible. Ansible can be run off of any machine it is installed on. It will use SSH to connect to the servers it manages. 
...
Ansible has a default inventory file used to define which servers it will be managing. There's an example one you can reference at `/etc/ansible/hosts`.
...
Then I create my own inventory file. In the following `/etc/ansible/hosts` file, we'll define two servers to manage under the "web" label.
...
For testing this example and learning Ansible, I actually created a virtual machine, installed Ansible on it, and then ran Ansible tasks directly on it. To do this, my `hosts` inventory file simply looked like this:
...
A consequence of this is that I need to tell Ansible to use "sudo", run tasks as user "Vagrant" and ask me the SSH password.
...
If that's setup, we can run commands directly:
...
This is a nice method for running a command on multiple servers simultaneously. For example, let's run an ad-hoc command on all servers:
...
If we use the modules, we can run these commands with idempotence. Above, we ran an ad-hoc command. This is **not** necessarily an idempotent action. There is an "apt" module we can use instead, which will run the same command but ensure idempotence. 
...
The result of running the task was `"changed": false`. This shows that there were no changes for me; I had already installed Nginx. Because this is an idempotent operation, I can run this command over and over without worrying about it changing the desired configuration.

We can run all of our needed commands ad-hoc, but let's make this more managable. We'll move this into a Playbook, which can run and coordinate multiple Tasks.

## Basic Playbook

[Playbooks](http://docs.ansible.com/playbooks_intro.html) can run multiple Tasks and provide some more advanced functionality that we would miss out on using ad-hoc commands. Configuration is all in Yaml - let's cover moving our task from above into a playbook.

We can create file `nginx.yml`:
...
This task does exactly the same as our ad-hoc command. We can run it with the `ansible-playbook` command:
...
Use use `-s` to tell Ansible to use `sudo` again, and then define the Playbook file.

We get some useful feedback, including what Ansible is doing. this way as well. Once again we see no changes were made - I have Nginx installed already.

### Handlers

Let's see how a Handler can improve this. A Handler is exactly the same as a Task (it can run anything a Task can), but it can be run as a result of another action. This is useful for "secondary" actions that might be required after running a task, such as starting a new service or reloading after a configuration change.
...
We've added a `notify` directive to the installation task. This notifies "Start Nginx".

Then we defined a handler called "Start Nginx". This Handler is the Task called when "Start Nginx" is notified.

The Handler uses the [Service module](http://docs.ansible.com/service_module.html), which can start, stop, restart, reload (and so on) system services. Here we simply tell Ansible that we want the service to be started.
...
We can uses Playbooks to run multiple tasks, add in variables, define other settings and even include other playbooks.
...
This playbook shows some more handy usage. We've shown some more functionality, but of course not all we can. For example, this doesn't show copying files or using templates.

Let's take Ansible further and by organizing the Playbook into a Role, and showing some extra functionality.
