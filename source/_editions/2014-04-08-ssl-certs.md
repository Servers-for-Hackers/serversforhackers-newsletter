...
Next, we need a place to put our certificates. I usually put them in the same configuration directories as the web server which I'm using them. For example, we can make a directory in the Apache or Nginx `/etc` directories:
...
Similar to the Apache setup, for using `xip.io` you can adjust the `server_name` and certificate paths and be on your way:
	...
	server {
    	...
    	ssl_certificate_key /etc/ssl/xip.io/xip.io.crt;
