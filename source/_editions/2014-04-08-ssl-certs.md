...
<!-- 
Info here: http://www.jupiterbroadcasting.com/54142/misconceptions-of-linux-security-techsnap-155/, ~ 51:45
Cert, Intermed Cert, Root Cert, other.
-->
...
If you're interested on learning more about [SSL certificates](http://en.wikipedia.org/wiki/Secure_Sockets_Layer) and the various mechanisms (such as "key certificate", "root certificate", "intermediate certificate"), jump to about `~51:45` of [this edition of Tech Snap](http://www.jupiterbroadcasting.com/54142/misconceptions-of-linux-security-techsnap-155/).
...
To start, first make sure you have OpenSSL installed. Most flavors of Linux have this, but you can usually easily install it.
...
Next, we need a place to put our certificates. I usually put them in the same configuration directories as the web server which I'm using them. For example, I'll usually make a directory in the Apache or Nginx `/etc` directories:
...
Once that's done, we can begin creating our certificate. First, we need a private key:
	...
	sudo openssl genrsa -out "/etc/[your-webserver-of-choice]/ssl/example.key" 2048
	
Then we can generate the Certificate Signing Request. This uses the Private Key generated before:

	sudo openssl req -new -key "/etc/apache2/ssl/example.key" -out "/etc/apache2/ssl/example.csr"

This will ask you a series of question. Here are the questions along with what I filled in:
...
> The `Common Name` option is the most important, as your domain used with the certificate needs match it. Commonly this means **specifying the "www" subdomain**, if you plan on using it, as well!

I left some blank. You can skip Organizational Unit Name and email address. I also choose to leave the optional company name blank. Finally, I elected NOT to add in a [challenge password](http://serverfault.com/questions/266232/what-is-a-challenge-password). This is something used by the certificate authority in the case you need to regenerate an SSL certiciate. Whether you use one or not in a production SSL is up to you.

So, we now have `example.key` and `example.csr` files created. Let's finish this up by creating the self-signed certificate.

	sudo openssl x509 -req -days 365 -in "/etc/apache2/ssl/example.csr" -signkey "/etc/apache2/ssl/example.key"  -out "/etc/apache2/ssl/example.crt"
...
* `-in "/etc/apache2/ssl/example.csr"` - The CSR generated for this certificate
* `-signkey "/etc/apache2/ssl/example.key"` - The Private Key used for this certificate
* `-out "/etc/apache2/ssl/example.crt"` - Where to put the new certificate file


### Creating a Wildcard Self-Signed Certificate

I use the [xip.io](http://xip.io) service very regularly so that I can avoid editing my hosts file for any virtual machine I spin up. It's useful to automate the process of creating a self-signed *wildcard* xip.io certificate for my local development machines. Let's see two things below:

1. Creating a certificate that will work with any subdomain
2. Generate a SSL certificate without human interaction, which we can use for scripting

Let's begin! Create a new shell script, we'll call it 'generate-ssl.sh`:

	#!/usr/bin/env bash
	
	# Create a generic "ssl" directory
...
The above script follows all of our previous steps, except it does some fancy bash scripting so we can automate passing in the CSR generating variables using the `-subj` flag.
...
To enable SSL for this site, we can create another virtual host file, or add another virtualhost block to our `example.conf` file. This will listen on port 443, a default https port.
...
And that's it! Once you have that in place and enabled, you can reload ngin (`sudo service apache2 reload`) and try it out!
...
Let's see what that looks like for the wildcard xip.io setup. The following virtualhost is for a web app located at `project-a.192.168.33.10.xip.io`, where "192.168.33.10" is the IP address of my server.
...
Here we can see the SSL certificate working, but of course our browser doesn't trust it since it's not certified by a trusted third party.
![xip.io wildcard subdomain](https://s3.amazonaws.com/serversforhackers/xip.io.ssl.png)

> "Server's certificate does not match the URL." - And here we thought we made a wildcard certificate! Well, it turns out that matching wildcards isn't supported the same way across implementations/browsers. [Read here](http://www.hanselman.com/blog/SomeTroubleWithWildcardSSLCertificatesFireFoxAndRFC2818.aspx) for some more information.
...
> You can see the process of creating an SSL certificate automated (no user input needed) in this [shell script](https://github.com/fideloper/Vaprobash/blob/master/scripts/base.sh#L22-L42) in the [Vaprobash project](https://github.com/fideloper/Vaprobash).
...
Use wget with a self-signed certificate:
	...
	wget --no-check-certificate -O OutputFilename https://mysapp.com/somefile